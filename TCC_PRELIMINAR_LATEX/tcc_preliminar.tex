...
\begin{itemize}
    \item \textbf{Modelo de \textit{Embedding}}: o modelo definitivo de representação vetorial ainda será selecionado a partir de testes empíricos adicionais na literatura e em benchmarks de embeddings. Para fins de validação e realização dos testes qualitativos preliminares deste relatório, adotou-se o modelo \texttt{sentence-transformers/all-MiniLM-L6-v2}, que codifica sentenças em vetores de 384 dimensões;
    ...
    \item \textbf{\textit{Framework} de Orquestração}: LangChain em ambiente Python, utilizado para a composição e a execução do pipeline, incluindo a cadeia de recuperação e a formatação do \textit{prompt} de geração.
\end{itemize}
